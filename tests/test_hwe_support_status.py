...
def make_mock_pkg(name, ver_str="1.0"):
    return hwe_support_status.Package(name, ver_str)
        ...
        with patch("hwe_support_status.is_unsupported_hwe_running") as m:
            with patch("__builtin__.print") as mock_print:
                ...
                hwe_support_status.advice_about_hwe_status(
                    mock_unsupported_hwe_pkgs, [], False, verbose=True)
        ...
        with patch("hwe_support_status.is_unsupported_hwe_running") as m:
            with patch("__builtin__.print") as mock_print:
                ...
                hwe_support_status.advice_about_hwe_status(
                    mock_unsupported_hwe_pkgs, [], False, verbose=True)
                text = mock_print.call_args[0][0]
                self.assertIn("You have packages from the Hardware Enablement "
                              "Stack (HWE) installed that", text)
    ...
    def test_advice_about_hwe_status_no_hwe_stack(self):
        with patch("hwe_support_status.is_unsupported_hwe_running") as m:
            with patch("__builtin__.print") as mock_print:
                ...
                self.assertIn(
                    "Your system is supported until April 2017", text)
    ...
    def test_advice_about_hwe_status_supported_hwe_stack(self):
        with patch("hwe_support_status.is_unsupported_hwe_running") as m:
            with patch("__builtin__.print") as mock_print:
                ...
                hwe_support_status.advice_about_hwe_status(
                    [], mock_supported_hwe_pkgs, False, verbose=True)
                text = mock_print.call_args[0][0]
                self.assertIn(
                    "Your Hardware Enablement Stack (HWE) is supported "
                    "until April 2017", text)
