...
''')
    ...
    def test_package_install_thirdparty_unsigned_indep(self):
        ...
        self.assertTrue('Package failed to install' in log)

            # if we are run as root, this might succeed, and we clean up again
        self.sandbox_oslib.remove_package('myppd', None)

    def test_package_install_local_unsigned_binary(self):
        '''package installation for unsigned local file:// repo, binary'''
        ...
        log = sandbox.log.getvalue()[log_offset:]
        self.assertFalse('no trusted origin' in log)
    ...
    def test_package_install_thirdparty_unsigned_binary(self):
        ...
        archive = self._create_archive()
        log_offset = sandbox.log.tell()
        httpd.start(18080, archive)
        try:
            self.sandbox_oslib.install_package('binary', None, 
                    'deb http://localhost:18080/ /', None)
            log = sandbox.log.getvalue()[log_offset:]
            self.assertTrue('no trusted origin' in log)
        finally:
            httpd.stop()
