...
#define UNITY_LAUNCHER_ALL_MONITORS_KEY UNITY_GCONF_OPTION_PATH"/num_launchers"
#define UNITY_STICKY_EDGE_KEY UNITY_GCONF_OPTION_PATH"/launcher_capture_mouse"
...
static gboolean unity_launcher_on_all_monitors (void);
...
cc_display_panel_finalize (GObject *object)
{
  ...
  iter = self->priv->gconf_notify_id;
  while (iter != NULL)
    {
      gconf_client_notify_remove(gconf_client_get_default (), (guint) iter->data);
      iter = g_slist_next (iter);
    }
  g_slist_free (self->priv->gconf_notify_id);
...
paint_output (CcDisplayPanel *self, cairo_t *cr, int i)
{
  ...
  if (is_unity_session ())
    {
      if (gnome_rr_output_info_is_active (output) && (unity_launcher_on_all_monitors () || gnome_rr_output_info_get_primary (output)))
        {
...
static void
stickyedge_widget_refresh (GtkSwitch *switcher)
{
  gboolean stickyedge_enabled = gconf_client_get_bool (gconf_client_get_default (), UNITY_STICKY_EDGE_KEY, NULL);

  gtk_switch_set_active (switcher, stickyedge_enabled);
}

static void
ext_stickyedge_changed_callback (GConfClient* client,
                                 guint cnxn_id,
                                 GConfEntry *entry,
                                 gpointer user_data)
{
  stickyedge_widget_refresh (GTK_SWITCH (user_data));
}
...
on_stickyedge_changed (GtkSwitch *switcher, GParamSpec *pspec, gpointer user_data)
{
  ...
  /* 3d */
  gconf_client_set_bool (gconf_client_get_default (), UNITY_STICKY_EDGE_KEY, enabled, NULL);
  /* 2d */
...
static gboolean
unity_launcher_on_all_monitors ()
{
  gint value = gconf_client_get_int (gconf_client_get_default (), UNITY_LAUNCHER_ALL_MONITORS_KEY, NULL);
  return (value == 0);
}
...
refresh_unity_launcher_placement (CcDisplayPanel *self)
{
  ...
  GList *connected_outputs = NULL;
  GList *list;
  gboolean launcher_on_all_monitors = unity_launcher_on_all_monitors ();
...
static void
ext_launcher_placement_changed_callback (GConfClient* client,
                                         guint cnxn_id,
                                         GConfEntry *entry,
                                         gpointer user_data)
{
  ...
  CcDisplayPanel *self = CC_DISPLAY_PANEL (user_data);
  gint launcher_gconf_value = 0;

  if (! switcher_set_to_launcher_on_all_monitors (self))
    launcher_gconf_value = 1;

  if (gconf_client_get_int (gconf_client_get_default (), UNITY_LAUNCHER_ALL_MONITORS_KEY, NULL) != launcher_gconf_value)
    refresh_unity_launcher_placement (self);
}
...
on_launcher_placement_combo_changed (GtkComboBox *combo, CcDisplayPanel *self)
{
  ...
  /* 3d */
  gconf_client_set_int (gconf_client_get_default (), UNITY_LAUNCHER_ALL_MONITORS_KEY, value, NULL);
  /* 2d */
...
setup_unity_settings (CcDisplayPanel *self)
{
  ...
  self->priv->gconf_notify_id = NULL;
  ...
  gconf_client_add_dir (gconf_client_get_default (), UNITY_GCONF_OPTION_PATH, 0, NULL);
  ...
  self->priv->gconf_notify_id = g_slist_append (self->priv->gconf_notify_id,
                                              GINT_TO_POINTER (
                                              gconf_client_notify_add (gconf_client_get_default (), UNITY_STICKY_EDGE_KEY,
                                                       ext_stickyedge_changed_callback, sticky_edge_switch,
                                                       NULL, NULL)));
  stickyedge_widget_refresh (GTK_SWITCH (sticky_edge_switch));

  g_signal_connect (G_OBJECT (WID ("launcher_placement_combo")), "changed",
              G_CALLBACK (on_launcher_placement_combo_changed), self);
  self->priv->gconf_notify_id = g_slist_append (self->priv->gconf_notify_id,
                                            GINT_TO_POINTER (
                                            gconf_client_notify_add (gconf_client_get_default (), UNITY_LAUNCHER_ALL_MONITORS_KEY,
                                                     ext_launcher_placement_changed_callback, self,
                                                     NULL, NULL)));
}
